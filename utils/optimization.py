...
def fit_online_logistic_estimate(
    ...
    step_size = (
        1 / (1 / 4 + 1 / (1 + diameter / 2)) * 0.75
    )  # slightly smaller step for stability
    iters = int(
        (1 / 0.75) * np.ceil((9 / 4 + diameter / 8) * np.log(diameter / precision))
    )
    ...
    for _ in range(iters):
        ...
        z_estimate = project_ellipsoid(
            x_to_proj=unprojected_update,
...
def project_ellipsoid(x_to_proj, ell_center, ecc_matrix, radius, safety_check=False):
    ...
    ell_dist_to_center = np.dot(
        x_to_proj - ell_center, np.linalg.solve(ecc_matrix, x_to_proj - ell_center)
    )
    ...
    # find proj
    lbda_opt = minimize_scalar(
        fun_proj, method="bounded", bounds=(0, 1000), options={"maxiter": 500}
    )
